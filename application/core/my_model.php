...
class MY_Model extends CI_Model
{
 ...
 //       if (!$this->_tabel) {
   //         $this->_tabel = 'tb_' . strtolower(str_replace('_model', '', get_class($this)));
    ...
    // Insert.
    public function insert($data,$_tabel,$date)
    {
        $data = (object)$data;
		if($date){
        $data->mod_date= date('Y:m:d');
		$data->mod_by=$this->session->userdata('username');}
        return $this->db->insert($_tabel, $data);
